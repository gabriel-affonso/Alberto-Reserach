...
To archive past work after Notion is enabled, run `set -a; source ~/.alberto-env; set +a; alberto notion backfill --db "$ALBERTO_DB"`. It is idempotent and includes every validated full-text, partial-text or abstract reading plus every historical digest candidate. Candidates are explicitly labeled `Candidate / metadata only`. When targeting an existing database, Alberto preserves its fields, renames its title field to `Article` when needed, and adds the missing archive fields automatically.
