...
To archive digest readings that existed before Notion was enabled, run `set -a; source ~/.alberto-env; set +a; alberto notion backfill --db "$ALBERTO_DB"`. It is idempotent and only includes readings that were actually represented in a digest. When targeting an existing database, Alberto preserves its fields, renames its title field to `Article` when needed, and adds the missing archive fields automatically.
