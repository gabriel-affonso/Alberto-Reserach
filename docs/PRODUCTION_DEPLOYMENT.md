...
After updating a NUC that already has prior digests, run `alberto notion backfill --db "$ALBERTO_DB"` once to archive its historical digest readings. The command can be safely re-run after an interruption.
